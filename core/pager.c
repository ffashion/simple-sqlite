...
int sqlitepager_open(
    Pager **new_pager,     /* Return the Pager structure here */
  ...
  *new_pager = NULL;
  ...
  pager = malloc(sizeof(*pager) + nameLen * 2 + 30);

  memset(pager, 0, sizeof(*pager) + nameLen * 2 + 30);
  if (pager == NULL) {
    sqliteOsClose(&fd);
    return SQLITE_NOMEM;
  }
  pager->zFilename = (char *)&pager[1];
  pager->zJournal = &pager->zFilename[nameLen + 1];
  strcpy(pager->zFilename, zFilename);
  strcpy(pager->zJournal, zFilename);
  strcpy(&pager->zJournal[nameLen], "-journal");
  pager->fd = fd;
  pager->journalOpen = 0;
  pager->ckptOpen = 0;
  pager->ckptInUse = 0;
  ...
  pager->ckptJSize = 0;
  pager->nPage = 0;
  pager->mxPage = mxPage > 5 ? mxPage : 10;
  ...
  pager->pLast = 0;
  pager->nExtra = nExtra;
  memset(pager->aHash, 0, sizeof(pager->aHash));
  *new_pager = pager;
  return SQLITE_OK;
}
...
*/
int sqlitepager_close(Pager *pager) {
  PgHdr *pPg, *pNext;
  switch (pager->state) {
  case SQLITE_WRITELOCK: {
    sqlitepager_rollback(pager);
    sqliteOsUnlock(&pager->fd);
    assert(pager->journalOpen == 0);
    break;
  }
  ...
  for (pPg = pager->pAll; pPg; pPg = pNext) {
    pNext = pPg->pNextAll;
    sqliteFree(pPg);
  }
  sqliteOsClose(&pager->fd);
  assert(pager->journalOpen == 0);
